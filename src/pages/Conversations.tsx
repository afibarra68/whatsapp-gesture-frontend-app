...
export function Conversations() {
  ...
  const showThreadMobile = Boolean(selectedId);
  ...
  return (
    <div className="chat-page">
      ...
      <div className="chat-layout">
        <aside
          className={`chat-sidebar${showListMobile ? ' chat-pane-visible' : ' chat-pane-hidden-mobile'}`}
        >
          <div className="chat-sidebar-head">
            <select value={filter} onChange={(e) => setFilter(e.target.value)} aria-label="Filtrar conversaciones">
              <option value="">Todas</option>
              <option value="bot">Bot</option>
              <option value="humano">Humano</option>
            </select>
            <span className="muted chat-count">{items.length}</span>
          </div>

          <div className="chat-list">
            {loading ? (
              <p className="muted chat-list-empty">Cargando…</p>
            ) : items.length === 0 ? (
              ...
              items.map((c) => (
                ...
                >
                  ...
                  <div className="chat-list-meta">
                    {c.cliente_nombre && <span className="mono">{c.telefono}</span>}
                    <Badge value={c.modo} />
                  </div>
        ...
        <section
          className={`chat-panel${showThreadMobile ? ' chat-pane-visible' : ' chat-pane-hidden-mobile'}`}
        >
          {!selected ? (
              ...
              <header className="chat-header">
                ...
                <div className="chat-header-actions row">
                  {isOpen(selected) ? (
                    ...
                    <span className="muted chat-window-closed">Ventana 24h cerrada</span>
                  )}
